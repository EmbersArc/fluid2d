...
if __name__ == "__main__":
    # Velocity
    v = np.zeros((GRIDSIZE, GRIDSIZE), dtype=np.float)
    v_prev = v.copy()
    u = np.zeros((GRIDSIZE, GRIDSIZE), dtype=np.float)
    u_prev = u.copy()

    # Density
    d = np.zeros((GRIDSIZE, GRIDSIZE), dtype=np.float)
    d_prev = d.copy()

    # Drawing
    WINDOW_SIZE = int((GRIDSIZE-2) * WINDOW_SCALING_FACTOR)
    window = pg.window.Window(WINDOW_SIZE, WINDOW_SIZE, caption="Fluid")
    ...
    def update(_):
        ...
        d_prev[GRIDSIZE//2, GRIDSIZE//16] = 10000.
        v_prev[GRIDSIZE//2, GRIDSIZE//16] = 50.
